...
%   result = pradar.runAnalysis(cfg, dataFile)
    ...
    %% RD dimensional compression and candidate extraction
    ...
    switch cfg.rdProjectionMode
        ...
        case "range"
            ...
            % ---------------------------------------------
            % Original method:
            % compress velocity dimension
            % preserve range dimension
            % ---------------------------------------------
        ...
        case "velocity"
            ...
            % ---------------------------------------------
            % Alternative method:
            % compress range dimension
            % preserve velocity dimension
            % ---------------------------------------------
            ...
            candidateTable = ...
                pradar.extractVelocityCandidates( ...
                    rdEnhancedVelocityTimeDb, ...
                    peakRangeMap, ...
                    targetData, ...
                    derived, ...
    ...
    result.rdProjectionMode = ...
        cfg.rdProjectionMode;

    result.rdEnhancedRangeTimeDb = ...
        rdEnhancedRangeTimeDb;

    result.aliasVelocityMap = ...
        aliasVelocityMap;
    ...
    if cfg.generateRdGif
        ...
        if needPowerGif
            ...
            %% Mean-removed visible color scale
            meanRemovedMaximum = max(meanFinite);
            
            meanRemovedVisibleClim = [ ...
                meanRemovedMaximum ...
                    - cfg.rdMeanRemovedVisibleDynamicRangeDb, ...
                meanRemovedMaximum];
            ...
            %% Mean-removed RD GIF with adaptive display scale

            if cfg.generateMeanRemovedRdGif
            
                meanVisibleGifPath = fullfile( ...
                    cfg.outputDir, ...
                    baseName + "_RD_mean_removed_visible.gif");
                ...
                pradar.createRdCubeGif( ...
                    ...
                    cfg, ...
                    meanVisibleGifPath, ...
                    "Short-time RD after mean removal - visible scale", ...
                    meanRemovedVisibleClim);
            
                result.gifPaths.meanRemovedVisible = ...
                    meanVisibleGifPath;
            end
        end
        ...
        if cfg.generateEnhancedRdGif
            ...
            pradar.createRdCubeGif( ...
                ...
                cfg, ...
                enhancedGifPath, ...
                "Background-enhanced short-time RD", ...
                cfg.rdGifClim);
