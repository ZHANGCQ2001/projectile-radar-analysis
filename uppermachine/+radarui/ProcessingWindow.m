...
%   keeping acquisition and legacy RD display code isolated from processing.
    ...
    methods (Access = private)
        ...
        function renderSummary(obj, result)
            ...
            lines{end + 1} = sprintf( ...
                '轨迹时间门: %.3f ~ %s ms', ...
                result.cfg.trackStartTimeMs, ...
                formatFiniteOrInf(result.cfg.trackEndTimeMs));
            lines{end + 1} = sprintf( ...
                '速度硬门限: %s；容差 %.1f m/s；最小|模糊速度| %.1f m/s', ...
                onOffText(result.cfg.useVelocityHardGate), ...
                result.cfg.velocityTolerance, ...
                result.cfg.highSpeedMin);
            lines{end + 1} = sprintf( ...
                '速度轴 Vmax: +/- %.3f m/s', ...
                result.derived.vMax);
            lines{end + 1} = sprintf( ...
...
function textValue = formatFiniteOrInf(value)
    if isfinite(value)
...
function textValue = onOffText(value)
    if value
        textValue = '开';
    else
        textValue = '关';
