...
%   keeping acquisition and legacy RD display code isolated from processing.
    ...
    methods (Access = private)
        function createComponents(obj)
            ...
            rdTab = uitab(obj.TabGroup, 'Title', '背景增强 RD');
            rdGrid = uigridlayout(rdTab, [1 1]);
            rdGrid.Padding = [8 8 8 8];
            obj.EnhancedRdAxes = uiaxes(rdGrid);
            title(obj.EnhancedRdAxes, '目标区间平均背景增强 RD');
            xlabel(obj.EnhancedRdAxes, 'Aliased Velocity (m/s)');
        ...
        function renderEnhancedRd(obj, result)
            cla(obj.EnhancedRdAxes);

            imagesc( ...
                obj.EnhancedRdAxes, ...
                result.derived.velocityAxis, ...
                result.derived.rangeAxis, ...
                result.rdEnhancedMeanDb);

            obj.EnhancedRdAxes.YDir = 'normal';
            xlabel(obj.EnhancedRdAxes, 'Aliased Velocity (m/s)');
            ylabel(obj.EnhancedRdAxes, 'Range (m)');
            title(obj.EnhancedRdAxes, ...
                '目标区间平均背景增强 RD (dB)');
